...
class Blip2Llama(Blip2Base):
    ...
    def __init__(
        ...
        self.prefix_embedding = nn.Parameter(torch.randn(1, self.prefix_length, self.gin_hidden_dim))
    ...
    def forward(self, batch):
        ...
        if True: # !!! llm and instruction
            ...
            else: # prefix fine-tune
                attention_mask = torch.cat([prefix_mask, instruction_tokens.attention_mask, graph_atts_mask], dim=1)
        else:
            graph_atts_mask = torch.ones(h_graph.size()[0], dtype=torch.long).to(device)
            graph_atts_mask = graph_atts_mask.unsqueeze(1)
            ...
            inputs_embeds = graph_inputs_llm
            attention_mask = graph_atts_mask
